...
GraphicFrameParser.processGraphicFrameNodes = (graphicFrames) => {
    ...
    for (const frame of graphicFrames) {
        const graphicTypeUri = attributesHandler_1.getAttributeByPath([frame], ["a:graphic", "a:graphicData", "$", "uri"]);
...
GraphicFrameParser.extractTableElements = (frame) => {
    const rawTable = attributesHandler_1.getAttributeByPath([frame], ["a:graphic", "a:graphicData", "a:tbl"]);
